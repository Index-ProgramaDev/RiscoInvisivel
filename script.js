...
async function verificarRadar() {
    ...
    try {
        ...
        console.error(e);
        container.innerHTML = "Erro ao consultar a API AlertaDengue. Verifique sua conexao e tente novamente.";
    }
}
...
document.getElementById('noticiaInput').addEventListener('keydown', e => { if(e.key === 'Enter') verificarNoticia(); });// ── Busca geocode IBGE dinamicamente (qualquer municipio do Brasil) ──
async function buscarGeocode(nomeCidade) {
    const query = encodeURIComponent(nomeCidade);
    const resp = await fetch(`https://servicodados.ibge.gov.br/api/v1/localidades/municipios?nome=${query}`);
    if (!resp.ok) throw new Error("Erro IBGE");
